...
% bad_tr_musi = [96,23,132,1,181,48,1,72,82,101,104,31,99,38,120,121,186,1,179,95]+1;
...
for w_tao = w
    for s = SS
        ...
        y    = y(ind); X = X(ind);
%         if s == 1 || s == 2 || s == 12
%             X    = cellfun(@(x) double(x(:,1:18)),X,'UniformOutput',false);
%         else
...
%                 end
        ...
        %         ylabel('Ventanas de Tiempo','Interpreter','latex')
        %         xlabel('$\lambda$ lasso','Interpreter','latex')
        %         saveas(gca,[SUBJECTS_DIR  filesep 'P_' num2str(s) filesep 'Sujeto '...
        %             num2str(s) '_acc_' experiment_name '_w' num2str((twin/fs)*1000) ...
        %             'msec'],'png')
        %% Saving results
        save([SUBJECTS_DIR filesep  's_' num2str(s) ...
            experiment_name '_w' num2str((twin/fs)*1000) 'msec_all'],...
...
for s = SS
    load([SUBJECTS_DIR filesep 's_' num2str(s)  'prueba_musica2_J_v3_w1000msec_all_3.mat'])
...
save([pwd filesep 'Main2_music3.mat'],'facc_')
figure(100)
plot(facc_')
