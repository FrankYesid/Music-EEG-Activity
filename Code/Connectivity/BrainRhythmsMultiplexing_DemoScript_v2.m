...
for sub = 1%:21
    ...
    rest_eeg = double(base);
    music_eeg= double(music.data);
    clear re_eeg_
    ...
    for i_song=1 % loop over songs
        ...
        singleSong_EEG = singleSong_EEG-repmat(mean(singleSong_EEG),19,1); % common-rereferencing
    ...
    %     for i_band=1:size(FBANDS,1)
    %         subplot(1,7,i_band)
    %         imagesc(Faxis([Freqs{i_band}(1):Freqs{i_band}(2)]),1:19,PSD_ryt{i_band}(:,[Freqs{i_band}(1):Freqs{i_band}(2)])) %#ok<NBRAK>
    %         if i_band == 1
    %             ylabel('Channel','Interpreter','latex')
    %         end
    %         xlabel('Frequency (f)','Interpreter','latex')
    %         title(FRR_names(i_band,:))
    %         set(gca,'YTick',1:19,'YTickLabel',name_channels,'TickLabelInterpreter','latex')
    %     end
    %     axis tight;
    %     set(gcf, 'units','normalized','outerposition',[0 0 1 1]); %EXPANDING FIGURE ON SCREEN
    %     hold off;
    %     saveas(fig,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure1'],'png')
    %     close
    ...
    %     set(gcf, 'units','normalized','outerposition',[0 0 1 1]); %EXPANDING FIGURE ON SCREEN
    %     hold off;
    %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure2'],'png')
    %     close
    ...
    %     set(gcf, 'units','normalized','outerposition',[0 0 1 1]); %EXPANDING FIGURE ON SCREEN
    %     hold off;
    %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure3'],'png')
    %     close
...
%     %     figure(4)
%     %     nch = ch; %take care of this variable (nch must be according to matrix size you want to plot it)
...
%     %     set(gcf, 'units','normalized','outerposition',[0 0 1 1]); %EXPANDING FIGURE ON SCREEN
%     %     hold off;
%     %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure4'],'png')
%     %     close
%     
%     %% topoplot
%     %     figure(5)
...
%     %         title(FRR_names(i,:))
%     %     end
%     %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure5'],'png')
%     %     close
%     
...
%     %     set(gcf, 'units','normalized','outerposition',[0 0 1 1]); %EXPANDING FIGURE ON SCREEN
%     %     hold off;
%     %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure6'],'png')
%     %     close
%     %
%     %% %STEP-4
%     %%%%%  Estimating Phase-Amplitude Coupling (PAC) for DELTA->BETA_high interaction
%     %
%     STEP=1; WINDOW=size(music_eeg,1)-1;% PAC Window = 8s
...
%                 clear REST_curve;
%                 REST_curve=squeeze(rest_eeg(1:9500,ELECTRODE))';
...
%                 for trial = 1:size(music_eeg,3)
%                     clear MUSIC_curve;
%                     MUSIC_curve=squeeze(music_eeg(:,ELECTRODE,trial))';
%                     Pf1=FBANDS(i1,1);  Pf2=FBANDS(i1,2);
%                     Af1=FBANDS(i2,1);  Af2=FBANDS(i2,2);
...
%     %     set(gca, 'XTick', 1:19,'XTickLabel',name_channels);grid,
%     %     saveas(gca,['D:\Dropbox\Paper BrainScore_BCMI\figures\sub',num2str(sub),'_figure7'],'png')
%     %     close
%     
% %     y = music.tempo;
% %     y(ismember(y,50)) = 1;
