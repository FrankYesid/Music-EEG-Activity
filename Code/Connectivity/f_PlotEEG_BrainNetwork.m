function [labels] = f_PlotEEG_BrainNetwork(nch, ijw, w_scal, groups, n_features, n_scal, cbtype)
% 
...
elseif nargin == 4
    ...
    w_scaling = w_scal;                 %taking the scaling of weigths
    n_atribut = ones(nch, 1);           %fixing nodes' attributes
    n_scaling = 'n_';              %fixing nodes' scaling
    cbnet = 'nocb';                     %fixing not to show the colorbars
...
elseif nchannels == 19    
    labels_ = labelsEEG{4};      %labels of a 64 montage
    labelsa = [1,3,9,11,13,15,17,30,32,34,50,52,54,62,64];
    labels_ = labels_(labelsa);
     labels = cell(19,1);
    for lab = 1:19
        if lab < 8
            labels{lab} = labels_{lab};
        elseif lab == 8
            labels{lab} = 'T3';
        elseif lab > 8 && lab < 12
            labels{lab} = labels_{lab-1};
        elseif lab == 12
            labels{lab} = 'T4';
        elseif lab == 13
            labels{lab} = 'T5';
        elseif lab > 13 && lab < 17
            labels{lab} = labels_{lab-3};
        elseif lab == 17
            labels{lab} = 'T6';
        elseif lab > 17
            labels{lab} = labels_{lab-4};
        end
    end
elseif nchannels == 14
    labels_ = labelsEEG{4};      %labels of a 64 montage
    % seleccionamos los canales
    n_chans = [6,9,11,26,28,48,62,64,56,36,20,15,17,8];
    for i = 1:nchannels
        labels{i,1} = labels_{n_chans(i)};
    end
else
...
[~,b] = (ismember(labels_, lay.label));   %b are the desired nodes' indexes from the gral layout
r_nodepos_ = lay.pos(b, :);      %position of desired nodes depending on the layout
if nchannels == 19
    for lab = 1:19
        if lab < 8
            r_nodepos(lab,:) = r_nodepos_(lab,:);
        elseif lab == 8
            r_nodepos(lab,:) = [-0.4022,r_nodepos_(lab,2)];
        elseif lab > 8 && lab < 12
            r_nodepos(lab,:) = r_nodepos_(lab-1,:);
        elseif lab == 12
            r_nodepos(lab,:) = [0.3986,r_nodepos_(lab-2,2)];
        elseif lab == 13
            r_nodepos(lab,:) = [-0.3639,-0.2796];
        elseif lab > 13 && lab < 17
            r_nodepos(lab,:) = r_nodepos_(lab-3,:);
        elseif lab == 17
            r_nodepos(lab,:) = [0.3639,-0.2796];
        elseif lab > 17
            r_nodepos(lab,:) = r_nodepos_(lab-4,:);
        end
    end
elseif nchannels == 14
    for lab = 1:14
        r_nodepos(lab,:) = r_nodepos_(n_chans(lab),:);
    end
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
switch n_scaling
    ...
    case 'n_'
        ...
        n_atribut_new = n_atribut.*0 + fixedsizenode;
        % % % associating equal nodes' attributes to ONE color
        RGBnodes = squeeze(double2rgb(n_atribut_new(1:8), colormap([210,180,28]./255)));
        for col = 1:7
            if col == 1
                RGBnodes(col,:) = [0,0.45,0.74];
            elseif col == 2
                RGBnodes(col,:) = [0.85,0.33,0.1];
            elseif col == 3
                RGBnodes(col,:) = [0.93,0.69,0.13];
            elseif col == 4
                RGBnodes(col,:) = [0.49,0.18,0.56];
            elseif col == 5
                RGBnodes(col,:) = [0.47,0.67,0.19];
            elseif col == 6
                RGBnodes(col,:) = [0.3,0.75,0.93];
            elseif col == 7
                RGBnodes(col,:) = [0.64,0.08,0.18];
            elseif col == 8
                RGBnodes(col,:) = [1,1,1];
            end
        end
...
for lk = 1 : size(ijw, 1)           %along all links
    ...
    for nd = 1 : 2                                  %for the two nodes of a link
        if nargin == 4
            if groups(ijw(lk,nd)) ~= 0
                plot(xynodes(nd, 1), xynodes(nd, 2), 'o', 'MarkerSize', 4*pi.*n_atribut_new(ijw(lk,nd)), ...
                'MarkerEdgeColor','k', 'MarkerFaceColor', RGBnodes(groups(ijw(lk,nd)), :), 'LineWidth',1.5);
            else
               plot(xynodes(nd, 1), xynodes(nd, 2), 'o', 'MarkerSize', 4*pi.*n_atribut_new(ijw(lk,nd)), ...
                'MarkerEdgeColor','k', 'MarkerFaceColor', RGBnodes(8, :), 'LineWidth',1.5); 
            end
        else
            plot(xynodes(nd, 1), xynodes(nd, 2), 'o', 'MarkerSize', 4*pi.*n_atribut_new(ijw(lk,nd)), ...
            'MarkerEdgeColor','k', 'MarkerFaceColor', RGBnodes(ijw(lk,nd), :), 'LineWidth',1.5);
        end
        text(xynodes(nd, 1)-0.04, xynodes(nd, 2)+0.01, labels{ijw(lk,nd)}, 'fontsize', 5);
