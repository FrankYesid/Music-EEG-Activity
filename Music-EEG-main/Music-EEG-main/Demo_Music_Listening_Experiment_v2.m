...
for i_band=1:size(FBANDS,1);
    ...
    [Px,Faxis]   = pwelch(filtered_EEG',[],[],[],Fs,'onesided'); %Welch method,128,100,256,
    Px = Px'; Px = Px(:,3:196); Faxis = Faxis(3:196); PSD_resting =Px; % Isolate frequency content in the [1-95]Hz range
    PLV_CP=[];[PLV_CP,IDX]  = PLV_multichannel_signal(filtered_EEG); %1 column vector (vectorized WA-matrix)
    REST_BAND_CPs(:,i_band) = PLV_CP;
end

% Music listening
for trial = 1:size(musig_eeg,3)
    MUSIC_BAND_CPs=[];  % [(29x28/2)=406 pairs x 7 Bands]
    for i_band=1:size(FBANDS,1);
        [b,a]=butter(3,[FBANDS(i_band,1),FBANDS(i_band,2)]/(Fs/2));
        EEG=music_eeg; %squeeze(music_eeg(:,:,trial));
        filtered_EEG=filtfilt(b,a,EEG')';
        [Px,Faxis]   = pwelch(filtered_EEG',[],[],[],Fs,'onesided'); %Welch method,128,100,256,
        Px = Px'; Px = Px(:,3:196); Faxis = Faxis(3:196); PSD_(trial,:,:) =Px; % Isolate frequency content in the [1-95]Hz range
        PLV_CP = [];[PLV_CP,IDX]=PLV_multichannel_signal(filtered_EEG); %1 column vector (vectorized WA-matrix)
        MUSIC_BAND_CPs(:,i_band)=PLV_CP;
    end
end
PSD_music = squeeze(mean(PSD_,1));
...
for i_band=1:size(FBANDS,1),
    subplot(1,7,i_band)
    imagesc(Faxis(),PSD_ryt{i_band}(:,[Freqs{i_band}(1):Freqs{i_band}(2)]))
end
